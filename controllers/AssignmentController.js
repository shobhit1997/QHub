...
async function addQuestions(req, res) {
    try {
        ...
        var assignment = await AssignmentManager.findAssignments({
            id: req.query.id,
        });
        ...
        var questions = await AssignmentManager.addQuestionsToAssignment(
            req.query.id,
            data
        );
        console.log(questions);
        if (questions) {
            await AssignmentManager.update(assignment.id, {
                question_count: assignment.question_count + questions.length,
            });
...
async function removeQuestions(req, res) {
    try {
        ...
        assignment = assignment[0];
        var questions = await AssignmentManager.removeQuestionsFromAssignment(
            req.query.id,
            data
        );
        if (questions) {
            await AssignmentManager.update(assignment.id, {
                question_count: assignment.question_count - questions,
            });
...
async function getQuestions(req, res) {
    try {
        var questions = await AssignmentManager.getQuestionsOfAssignment(
            req.query.id
        );
        if (questions) {
            res.send(questions);
...
async function generateAssignment(req, res) {
    try {
        if (!req.query.id) {
            return res
                .status(400)
                .send({ message: "Please provide an assignment id" });
        }
        var assignment = await AssignmentManager.findAssignments({
            id: req.query.id,
        });
        ...
        assignment = assignment[0];
        course_outcomes = await SubjectManager.getCourseOutcomes(
        ...
        var filePath = path.resolve(
            __dirname,
            `../outputAssignments/${assignment.subject_name}_Assignment ${assignment.assignment_no}.docx`
        );
...
async function uploadAssignment(req, res) {
    try {
        ...
        var filePath = path.resolve(
            __dirname,
            `../outputAssignments/${assignment.fileName}`
        );
        if (!fs.existsSync(filePath)) {
            res
                .status(400)
                .send({ message: "Please generate assignment before uploading" });
        }
