...
async function addQuestions(req,res){
    try{
        ...
        var question_paper=await QuestionPaperManager.findQuestionPaperSections({id:section_id,question_paper_id:req.query.id});
        if(!question_paper || question_paper.length==0){
            return res.status(400).send({message:"Invalid QuestionPaper/Section Id"});
        }
        question_paper=question_paper[0];
        var questions=await QuestionPaperManager.addQuestionsToQuestionPaper(section_id,data);
        if(questions){
...
async function removeQuestions(req,res){
    try{
        ...
        question_paper=question_paper[0];
        var questions=await QuestionPaperManager.removeQuestionsFromQuestionPaper(section_id,data);
        if(questions){
...
async function get(req,res){
    try{
        var question_paper=await QuestionPaperManager.findQuestionPapers(R.pick(['id','subject_code','subject_id','unit_id','unit_no','question_paper_no'],req.query))
        if(question_paper){
            res.send(question_paper)
        }
        else{
...
async function generateQuestionPaper(req,res){
    try{
        if(!req.query.id){
            return res.status(400).send({message:"Please provide an question_paper id"});
        }
        var question_paper=await QuestionPaperManager.findQuestionPapers({id:req.query.id});
        if(!question_paper || question_paper.length==0){
        ...
        course_outcomes=SubjectManager.getCourseOutcomes(question_paper.subject_id);
        question_paper.course_outcomes=course_outcomes;
        await docs.generateQuestionPaper(question_paper);
        var filePath= path.resolve(__dirname, `../outputQuestionPapers/${question_paper.subject_name}_Exam ${question_paper.exam}.docx`)
