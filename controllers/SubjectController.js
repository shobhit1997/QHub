...
async function create(req, res) {
    try {
        ...
        if (e.code == "ER_DUP_ENTRY") {
            res
                .status(400)
                .send({ message: `Duplicate Subject Code(${req.body.subject_code})` });
        }
        res.sendStatus(500);
    }
}
async function get(req, res) {
    try {
        var subject = await SubjectManager.findSubject(
            R.pick(["id", "subject_code"], req.query)
        );
        if (subject) {
            res.send(subject);
        } else {
...
async function addCourseOutcomes(req, res) {
    try {
        ...
        if (!subject || subject.length == 0) {
            return res
                .status(400)
                .send({ message: "Please provide a valid subject id" });
        }
        outcomes = req.body;
