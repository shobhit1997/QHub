...
async function createQuestionPaper(data){
    console.log(data);
    question_paper_data=R.pick(["exam","type","branch","year","created_by","subject_id","date_of_exam"],data);
    sections_data=data.sections;
    units_data=data.units;
    var question_paper_id=await QuestionPaper.createQuestionPaper(question_paper_data);
    if(question_paper_id && question_paper_id.length>0){
        ...
        if(data.type==="automatic"){
            ...
            var questions = await Question.pickRandomQuestionsForQuestionPaper(sections_data,units_data);
...
async function findQuestionPapers(searchData){
    ...
    if(question_paper){
        ...
        question_paper=question_paper.map(async as=>{
            console.log(as);
            as.sections=await QuestionPaper.getQuestionPaperSections({question_paper_id:as.id});
            as.sections=await Promise.all(as.sections.map(async s=>{
                console.log(s);
                s.questions=await QuestionPaper.getSectionQuestions(s.id);
                return s;
            }));
            as.units=await QuestionPaper.getPaperUnits({question_paper_id:as.id});
            return as;
        })
